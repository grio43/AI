...
using SharedComponents.EVE.Types;
...
namespace SharedComponents.IPC
{
    ...
    public interface IOperationContract
    {
        ...
        [OperationContract]
        int GetDumpLootIterations(string GUID);

        [OperationContract]
        void IncreaseDumpLootIterations(string GUID);
        ...
        [OperationContract]
        EveAccount GetEveAccount(string GUID);

        [OperationContract]
        void CompileEveSharpCore(string GUID);

        [OperationContract(IsOneWay = true)]
        void SetEveAccountAttributeValue(string GUID, string attributeName, object val);

        [OperationContract]
        void SetEveAccountAttributeValueBlocking(string GUID, string attributeName, object val);
        ...
        [OperationContract(IsOneWay = true)]
        void OnDirectEvent(string charName, string GUID, DirectEvent directEvent);

        [OperationContract(IsOneWay = true)]
        void AttachCallbackHandlerToEveAccount(string GUID);
    ...
    public class OperationContract : IOperationContract
    {
        ...
        public void SetEveAccountAttributeValueBlocking(string GUID, string attributeName, object val)
        {
            if (string.IsNullOrEmpty(GUID))
            {
                Cache.Instance.Log("SetEveAccountAttributeValueBlocking: GUID IsNullOrEmpty?!");
            }

            SetEveAccountAttributeValue(GUID, attributeName, val);
        }

        public void SetEveAccountAttributeValue(string GUID, string attributeName, object val)
        {
            try
            {
                if (string.IsNullOrEmpty(GUID))
                {
                    Cache.Instance.Log("SetEveAccountAttributeValue: GUID IsNullOrEmpty?!");
                }

                if (Util.IntervalInMilliseconds(30000, 30000, GUID + attributeName + val))
                {
                    var eA = GetEveAccount(GUID);
                    if (eA != null)
                    {
                        var t = eA.GetType();
        ...
        public void CompileEveSharpCore(string GUID)
        {
            Util.RunInDirectory("Updater.exe", "CompileEVESharpCore");
        }

        public EveAccount GetEveAccount(string GUID)
        {
            try
            {
                if (string.IsNullOrEmpty(GUID))
                {
                    Cache.Instance.Log("GetEveAccount: GUID: [" + GUID + "] IsNullOrEmpty?!");
                }

                if (_eveAccountCache.TryGetValue(GUID, out var eA) && eA != null)
                {
                ...
                else
                {
                    var ret = Cache.Instance.EveAccountSerializeableSortableBindingList.List.ToList()
                        .FirstOrDefault(s => !string.IsNullOrEmpty(s.GUID) && s.GUID.Equals(GUID));
                    if (ret != null)
                        _eveAccountCache[GUID] = ret; // AddOrUpdate func isn't atomic!
            ...
            catch (Exception e)
            {
                Console.WriteLine(e);
                Cache.Instance.Log(e.ToString());
                Cache.Instance.Log("GUID: [" + GUID + "] Not Found?!?");
                return null;
            }
        }
        ...
        public int GetDumpLootIterations(string GUID)
        {
            if (string.IsNullOrEmpty(GUID))
            {
                Cache.Instance.Log("GetDumpLootIterations: GUID IsNullOrEmpty?!");
            }
        ...
        public Dictionary<int, SolarSystemEntry> GetGateCampInfo(int[] solarSystemIds, TimeSpan? cacheDuration)
        {
            ...
            **/
            ...
            return new Dictionary<int, SolarSystemEntry>();
        }
        ...
        public void IncreaseDumpLootIterations(string GUID)
        {
            if (string.IsNullOrEmpty(GUID))
            {
                Cache.Instance.Log("IncreaseDumpLootIterations: GUID IsNullOrEmpty?!");
            }
        ...
        public void OnDirectEvent(string charName, string GUID, DirectEvent directEvent)
        {
            try
            {
                DirectEventHandler.OnNewDirectEvent(charName, GUID, directEvent);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public void AttachCallbackHandlerToEveAccount(string GUID)
        {
            try
            {
                if (string.IsNullOrEmpty(GUID))
                {
                    Cache.Instance.Log("AttachCallbackHandlerToEveAccount: GUID IsNullOrEmpty?!!");
                }
