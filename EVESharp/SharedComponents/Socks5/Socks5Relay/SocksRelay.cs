    ...
    namespace SocksRelay
    {
        ...
        class SocksRelay : IDisposable
        {
            ...
            private async Task Listen()
            {
                ...
                {
                    var client = await _localListener.AcceptTcpClientAsync();
            ...
            private async Task<TcpClient> SetupRemoteClient()
            {
                ...
                await remoteClientStream.WriteAsync(authRequest, 0, authRequest.Length);
            ...
            private async Task HandleClient(TcpClient client)
            {
                try
                {
                    ...
                    {
                        await clientStream.CopyToAsync(remoteClientStream, 81920, clientCancellationTokenLinked.Token);
                    }

                    async Task ServerToClient()
                    {
                        await remoteClientStream.CopyToAsync(clientStream, 81920, clientCancellationTokenLinked.Token);
                    }
