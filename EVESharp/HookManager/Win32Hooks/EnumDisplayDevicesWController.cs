...
namespace HookManager.Win32Hooks
{
    ...
    /// </summary>
    public class EnumDisplayDevicesWController : IHook, IDisposable
    {
        #region Fields
        ...
        public EnumDisplayDevicesWController(HWSettings hWSettings)
        {
            Error = false;
            ...
            try
            {
                _hook = LocalHook.Create(
                    LocalHook.GetProcAddress("user32.dll", "EnumDisplayDevicesW"),
        ...
        private bool EnumDisplayDevicesDetour(string lpDevice, uint iDevNum, ref DISPLAY_DEVICE lpDisplayDevice, uint dwFlags)
        {
            ...
            var orig = EnumDisplayDevicesW(lpDevice, iDevNum, ref lpDisplayDevice, dwFlags);
            if (lpDisplayDevice.StateFlags.HasFlag(DisplayDeviceStateFlags.PrimaryDevice))
            {
                ...
                if (lpDisplayDevice.DeviceID.Contains("DEV_"))
                {
                    ...
                    //Util.GlobalRemoteLog($"EnumDisplayDevicesWDetour Orig [{substring}] Spoofed [{replacement}]");
                    //WCFClient.Instance.GetPipeProxy.RemoteLog("DeviceString " + lpDisplayDevice.DeviceString);
