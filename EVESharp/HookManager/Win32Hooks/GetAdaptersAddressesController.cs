...
{
    public class GetAdaptersAddressesController : IDisposable, IHook
    {
        #region Fields
        ...
        public GetAdaptersAddressesController(IntPtr address, string guid, string mac, string ipaddress, string adapterName)
        {
            ...
            try
            {
                ...
                _name = string.Format("GetAdaptersAddressesHook_{0:X}", address.ToInt64());
                _hook = LocalHook.Create(address, new GetAdaptersAddressesDelegate(GetAdaptersAddressesDetour), this);
        ...
        private delegate ulong GetAdaptersAddressesDelegate(uint Family, uint Flags, IntPtr Reserved, IntPtr pAdapterAddresses, ref uint pOutBufLen);
        ...
        private static extern ulong GetAdaptersAddresses(uint Family, uint Flags, IntPtr Reserved, IntPtr pAdapterAddresses, ref uint pOutBufLen);
        ...
        private ulong GetAdaptersAddressesDetour(uint Family, uint Flags, IntPtr Reserved, IntPtr pAdapterAddresses, ref uint pOutBufLen)
        {
            try
            {
                //HookManagerImpl.Log("GetAdaptersAddressesDetour proc!");
            var k = GetAdaptersAddresses(Family, Flags, Reserved, pAdapterAddresses, ref pOutBufLen);

            if (pAdapterAddresses != IntPtr.Zero)
            {

                    var structBefore =
                        (IP_ADAPTER_ADDRESSES)Marshal.PtrToStructure(pAdapterAddresses, typeof(IP_ADAPTER_ADDRESSES));
                structBefore.Next = IntPtr.Zero; // force 1 adapter
                structBefore.PhysicalAddressLength = 6;

                // after
                structBefore.AdapterName = "{" + _guid.ToUpper() + "}";
                    structBefore.Description = _adapterName;

                    var n = 0;
                    for (var i = 0; n < structBefore.PhysicalAddressLength; i += 2)
                    {
                        structBefore.PhysicalAddress[n] =
                            Convert.ToByte(_mac[i].ToString() + _mac[i + 1].ToString(), 16);
                        n++;
                    }
                    structBefore.FirstUnicastAddress = IntPtr.Zero;
                    Marshal.StructureToPtr(structBefore, pAdapterAddresses, false);
                }
                return k;
                }
                catch (Exception e)
                {
                Debug.WriteLine(e);
                    HookManagerImpl.Instance.SendToInjectorLog($"{e}");
