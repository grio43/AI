    ...
    public partial class ESCache
    {
        ...
        public EveAccount EveAccount
        {
            get
            {
                if (_eveAccount == null)
                    try
                    {
                        Console.WriteLine("WCFClient.Instance.GUID [" + WCFClient.Instance.GUID + "]");

                        if (string.IsNullOrEmpty(WCFClient.Instance.GUID))
                        {
                            Console.WriteLine("EveAccount: WCFClient.Instance.GUID IsNullOrEmpty?!?!");
                        }

                        Console.WriteLine("GetEveAccount using GUID");
        ...
        public List<ModuleCache> Weapons
        {
            get
            {
                if (_weapons == null)
                {
                    _weapons = Modules.Where(m => m.IsOnline && m.IsWeapon).ToList() ?? new List<ModuleCache>();
        ...
        public bool CloseEve(bool restart, string reason)
        {
            ...
            if (restart) ESCache.Instance.TaskSetEveAccountAttribute(nameof(EveAccount.RestartOfEveClientNeeded), restart);
