                ...
                case MaterialsForWarPreparationArmState.MoveOreToShip:
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            if (ESCache.Instance.CurrentShipsCargo.Capacity > 0 && ESCache.Instance.CurrentShipsCargo.Capacity - ESCache.Instance.CurrentShipsCargo.UsedCapacity < volumeToMove)
                            {
                                Log.WriteLine("Transport ship has not enough free space. total cargospace [" + Math.Round(ESCache.Instance.CurrentShipsCargo.Capacity, 2) + "] Used [" + Math.Round(ESCache.Instance.CurrentShipsCargo.UsedCapacity, 2) + "] vol to move [" + Math.Round(volumeToMove, 2) + "]: Declining Mission.");
                                return StorylineState.RemoveOffer;
                            }
