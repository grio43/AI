...
namespace EVESharpCore.Questor.Behaviors
{
    public class MiningBehavior
    {
        ...
        private static void HandleOreWeHaveAlreadyMined()
        {
            ...
            {
                if (DebugConfig.DebugMiningBehavior) Log.WriteLine("HandleOreWeHaveAlreadyMined: We are in station");
                ChangeMiningBehaviorState(MiningBehaviorState.Start);
                return;
            }

            if (ESCache.Instance.CurrentShipsOreHold == null)
            {
                Log.WriteLine("HandleOreWeHaveAlreadyMined: if (ESCache.Instance.CurrentShipsOreHold == null)");
                return;
            }

            if (ESCache.Instance.CurrentShipsOreHold.UsedCapacityPercentage > 90 || 50 > ESCache.Instance.CurrentShipsOreHold.FreeCapacity)
            {
                if (DebugConfig.DebugMiningBehavior) Log.WriteLine("HandleOreWeHaveAlreadyMined: We are full: [" + ESCache.Instance.CurrentShipsOreHold.FreeCapacity + "] which is [" + ESCache.Instance.CurrentShipsOreHold.UsedCapacityPercentage + "]% limit is 90%");
                //go home to dump ore
                ChangeMiningBehaviorState(MiningBehaviorState.MakeBookmarkForNextOreRoid);
                return;
            }

            if (DebugConfig.DebugMiningBehavior) Log.WriteLine("HandleOreWeHaveAlreadyMined: We still have room: [" + ESCache.Instance.CurrentShipsOreHold.FreeCapacity + "] which is only [" + ESCache.Instance.CurrentShipsOreHold.UsedCapacityPercentage + "]% limit is 90%");
            return;
        }
        ...
        private static bool GotoNextAsteroidBeltOrSite()
        {
            ...
            if (AllowMiningInMiningAnomolies && ESCache.Instance.Modules.Any(i => i.IsOreMiningModule)) //Any Anomolies in local? Any mining anomolies?)
            {
                //
            ...
            if (AllowMiningInIceSignatures && ESCache.Instance.Modules.Any(i => i.IsIceMiningModule)) //Any Signatures in local? Any mining signatures?)
            {
                //
            ...
            if (AllowMiningInIceAnomolies && ESCache.Instance.Modules.Any(i => i.IsIceMiningModule)) //Any Anomolies in local? Any mining anomolies?)
            {
                //
            ...
            if (AllowMiningInAsteroidBelts && NextAvailableAsteroidBeltInLocal != null)
            {
        ...
        private static bool EveryPulse()
        {
            ...
            if (!ESCache.Instance.InSpace && ESCache.Instance.InStation)
            {
                ...
                {
                    if (DebugConfig.DebugMiningBehavior) Log.WriteLine("InStation and State.CurrentMiningBehaviorState [" + State.CurrentMiningBehaviorState + "] Forcing change to Start");
                    ChangeMiningBehaviorState(MiningBehaviorState.Start);
                }
            }
