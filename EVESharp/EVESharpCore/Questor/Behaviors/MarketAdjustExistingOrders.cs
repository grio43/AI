...
namespace EVESharpCore.Questor.Behaviors
{
    public class MarketAdjustBehavior
    {
        ...
        private static Dictionary<long, double> MyOriginalMarketOrderPrice = new Dictionary<long, double>();
        private const int _orderModificationDelayInMinutes = 30;
        ...
        private static int BuyOrderDoNotAdjustOrdersAboveThisPercentageOverTheLifeOfTheOrder = 50;
        private static int BuyOrderDoNotAdjustOrdersAboveThisPercentagePerChange = 30;
        private static int BuyOrderRandomSmallPercentageHigh = 20;

        private static int BuyOrderRandomSmallPercentageLow = 5;

        private static bool CheckBuyOrders;

        private static bool CheckSellOrders;

        private static IEnumerable<DirectOrder> myBuyOrders;

        private static IEnumerable<DirectOrder> mySellOrders;

        private static int PublicOrdersWithThisLowVolumeRemainingAreIgnored = 1;

        private static int SellOrderDoNotAdjustOrdersAboveThisPercentageOverTheLifeOfTheOrder = 50;

        private static int SellOrderDoNotAdjustOrdersAboveThisPercentagePerChange = 30;

        private static int SellOrderRandomSmallPercentageHigh = 20;

        private static int SellOrderRandomSmallPercentageLow = 5;
        ...
        private static int BuyOrderRandomSmallPercentage
        {
            get
            {
                ...
                {
                    _buyOrderRandomSmallPercentage = ESCache.Instance.RandomNumber(BuyOrderRandomSmallPercentageLow, BuyOrderRandomSmallPercentageHigh);
        ...
        private static int SellOrderRandomSmallPercentage
        {
            get
            {
                ...
                {
                    _sellOrderRandomSmallPercentage = ESCache.Instance.RandomNumber(SellOrderRandomSmallPercentageLow, SellOrderRandomSmallPercentageHigh);
        ...
        public static bool ChangeMarketAdjustBehaviorState(MarketAdjustBehaviorState stateToSet, bool wait = false, string LogMessage = null)
        {
            try
            {
        ...
        public static void LoadSettings(XElement CharacterSettingsXml, XElement CommonSettingsXml)
        {
            ...
            PublicOrdersWithThisLowVolumeRemainingAreIgnored =
            ...
            SellOrderDoNotAdjustOrdersAboveThisPercentageOverTheLifeOfTheOrder =
            ...
            SellOrderDoNotAdjustOrdersAboveThisPercentagePerChange =
            ...
            SellOrderRandomSmallPercentageLow =
            ...
            SellOrderRandomSmallPercentageHigh =
            ...
            BuyOrderDoNotAdjustOrdersAboveThisPercentageOverTheLifeOfTheOrder =
            ...
            BuyOrderDoNotAdjustOrdersAboveThisPercentagePerChange =
            ...
            BuyOrderRandomSmallPercentageLow =
            ...
            BuyOrderRandomSmallPercentageHigh =
        ...
        private static void CheckBuyOrdersState()
        {
            ...
            if (myBuyOrders.Any())
            {
                ...
                foreach (DirectOrder myBuyOrder in myBuyOrders)
                {
                    ...
                    {
                        if (MyOriginalMarketOrderPrice.Any(i => i.Key != myBuyOrder.OrderId))
                            MyOriginalMarketOrderPrice.Add(myBuyOrder.OrderId, myBuyOrder.Price);
                    }
                    else
                    {
        ...
        private static void CheckSellOrdersState()
        {
            ...
            if (mySellOrders == null)
            {
                if (DebugConfig.DebugMarketOrders) Log.WriteLine("CheckSellOrdersMAEOState: if (mySellOrders == null)");
                return;
            }

            if (DebugConfig.DebugMarketOrders) Log.WriteLine("CheckSellOrdersMAEOState: We have [" + mySellOrders.Count() + "] Sell Orders");

            if (mySellOrders != null && mySellOrders.Any())
            {
                int sellOrderNumber = 0;
                foreach (DirectOrder mySellOrder in mySellOrders)
                {
                    sellOrderNumber++;
                    if (!MarketWindow.IsReady)
                    {
                    ...
                    **/
                    ...
                    IOrderedEnumerable<DirectOrder> allSellOrdersLowerThanMyOrder = MarketWindow.SellOrders.Where(o =>
                        ...
                        mySellOrders.All(order => order.OrderId != o.OrderId) &&
                        o.VolumeRemaining > PublicOrdersWithThisLowVolumeRemainingAreIgnored).OrderByDescending(i => i.Price);

                    //
                    // so that we do not have to calc skills only deal with orders made in the local station
                    //
                    if (mySellOrder.StationId == ESCache.Instance.DirectEve.Session.StationId)
                        if (allSellOrdersLowerThanMyOrder.Any())
                        {
                            ...
                            if (publicSellOrderLowerThanMine != null)
                            {
                                ...
                                if (!IsLowestPublicSellOrderWithinPercentage(mySellOrder, publicSellOrderLowerThanMine, SellOrderDoNotAdjustOrdersAboveThisPercentagePerChange, sellOrderNumber)) continue;
        ...
        private static bool? HasItBeenLongEnoughSinceOurLastCheckOfThisOrder(DirectOrder myOrder, int OrderNumber)
        {
            if (Time.Instance.LastMarketOrderAdjustmentTimeStamp != null)
            {
                ...
                if (Time.Instance.LastMarketOrderAdjustmentTimeStamp.ContainsKey(myOrder.OrderId))
                    if (DateTime.UtcNow < Time.Instance.LastMarketOrderAdjustmentTimeStamp[myOrder.OrderId].AddMinutes(_orderModificationDelayInMinutes))
                    {
                ...
                if (Time.Instance.LastMarketOrderCheckTimeStamp.ContainsKey(myOrder.OrderId))
                    if (DateTime.UtcNow < Time.Instance.LastMarketOrderCheckTimeStamp[myOrder.OrderId].AddMinutes(_orderModificationDelayInMinutes))
                    {
        ...
        private static void PullMyBuyOrdersState()
        {
            if (myBuyOrders != null)
            {
                Log.WriteLine("PullMyBuyOrdersMAEOState: We have [" + myBuyOrders.Count() + "] buy orders: _nextMarketOrdersUpdate [" + _nextMarketOrdersUpdate.ToShortTimeString() + "]");
            ...
            myBuyOrders = MarketWindow.GetMyOrders(GetBuyOrdersTrue);
        }
        ...
        private static void PullMySellOrdersState()
        {
            if (mySellOrders != null)
            {
                Log.WriteLine("PullMySellOrdersMAEOState: We have [" + mySellOrders.Count() + "] sell orders: _nextMarketOrdersUpdate [" + _nextMarketOrdersUpdate.ToShortTimeString() + "]");
            ...
            mySellOrders = MarketWindow.GetMyOrders(GetBuyOrdersFalse);
        }
