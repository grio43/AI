        ...
        **/
        ...
        private static bool HandleWeaponsNeedReload()
        {
            try
            {
                ...
                if (ESCache.Instance.Weapons.Where(i => i.IsTurret || i.IsMissileLauncher).All(i => i.ChargeQty > i.MaxCharges - 8))
                {
                    intCount = 0;
                ...
                if (ESCache.Instance.Weapons.Where(i => i.IsTurret || i.IsMissileLauncher).Any(weapon => (weapon._module.IsMaster || ESCache.Instance.Weapons.All(x => !x._module.IsMaster)) && 1 >= weapon.ChargeQty && !weapon.IsReloadingAmmo && weapon.IsOnline && !weapon.IsActive && !weapon.IsDeactivating && !weapon.IsInLimboState))
                {
                    ...
                    else
                    {
                        ...
                        foreach (ModuleCache thisWeapon in ESCache.Instance.Weapons.Where(i => i.IsReadyToReloadAmmo))
                        {
                            ...
                            intCount++;
                            Combat.Combat.ReloadAmmo(thisWeapon, intCount);
                        }
                    }
                }
