...
namespace EVESharpCore.Questor.BackgroundTasks
{
    public static class Defense
    {
        ...
        private static void ActivateArmorRepairModules()
        {
            ...
            ModuleNumber = 0;
            foreach (ModuleCache armorRepairModule in ESCache.Instance.Modules.Where(i => i.GroupId == (int) Group.ArmorRepairer)
                .Where(x => x.IsOnline).OrderByDescending(a => a._module.Attributes.TryGet<int>("armorDamageAmount")))
            {
                ...
                {
                    if (DebugConfig.DebugDefense)
                        Log.WriteLine("[" + ModuleNumber + "][" + armorRepairModule.TypeName + "] is InLimboState, continue");
                    continue;
                }
                     ...
                     (!inCombat && perc < DeactivateRepairModulesAtThisPerc && cap > Panic.SafeCapacitorPct)))
                {
                    ...
                    if (ModuleNumber == 1)
                    {
                        ...
                        if (armorRepairModule.Click())
                        {
                            ...
                            perc = ESCache.Instance.ActiveShip.ArmorPercentage;
                            Log.WriteLine("Tank % [" + Math.Round(perc, 0) + "%] Cap: [" + Math.Round(cap, 0) + "%] Armor Repairer: [" +
                    ...
                    else if (perc < ActivateSecondRepairModulesAtThisPerc)
                    {
                        ...
                        if (DebugConfig.DebugDefense)
                            Log.WriteLine("Perc: [" + Math.Round(perc, 0) + "%] Cap: [" + Math.Round(cap, 0) + "%] Attempting to Click to Deactivate [" +
                                          ModuleNumber +
                                          "][" + armorRepairModule.TypeName + "].");
                        if (armorRepairModule.Click())
                        {
                ...
                if (armorRepairModule.IsActive && perc >= DeactivateRepairModulesAtThisPerc)
                {
                    if (DebugConfig.DebugDefense)
                    ...
                    if (armorRepairModule.Click())
                    {
                        ...
                        perc = ESCache.Instance.ActiveShip.ArmorPercentage;
                        Log.WriteLine("Tank %: [" + Math.Round(perc, 0) + "%] Cap: [" + Math.Round(cap, 0) + "%] Armor Repairer: [" + ModuleNumber +
                                      "] deactivated [" +
        ...
        private static void ActivateHardeners()
        {
            ...
            ModuleNumber = 0;
            foreach (ModuleCache hardener in ESCache.Instance.Modules.Where(i => i.GroupId == (int) Group.ShieldHardeners || i.GroupId == (int) Group.ArmorHardeners || (i.GroupId == (int) Group.ShieldBoosters && Defense.ActivateRepairModulesAtThisPerc > 100 && Combat.Combat.PotentialCombatTargets.Any())))
            {
            ...
            try
            {
                try
                {
                    ...
                    if (ESCache.Instance.InMission && MissionSettings.MyMission != null && MissionSettings.MyMission.Name.Contains("Anomic Team"))
                        ...
                        {
                            if (DebugConfig.DebugOverLoadWeapons) Log.WriteLine("DebugOverLoadhardeners: We are doing an Anomic Team mission and are within 60 sec of dropping out of warp. Do not yet overload.");
                            return;
                        }
            ...
            try
            {
                try
                {
                    ...
                    if (ESCache.Instance.InMission && MissionSettings.MyMission != null && MissionSettings.MyMission.Name.Contains("Anomic Team"))
                        if (Time.Instance.LastInWarp.AddSeconds(45) > DateTime.UtcNow)
                        {
                            if (DebugConfig.DebugOverLoadWeapons) Log.WriteLine("DebugOverLoadReps: We are doing an Anomic Team mission and are within 60 sec of dropping out of warp. Do not yet overload.");
                            return;
                        }
