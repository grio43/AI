            ...
            foreach (DirectWindow modalWindow in ESCache.Instance.DirectEve.ModalWindows)
            {
                ...
                **/
                ...
                if (modalWindow.IsModal || modalWindow.Name == "modal")
                    ...
                    {
                        gotoBaseNow |= modalWindow.Html.Contains("for a short unscheduled reboot");
                        disableInstance |= modalWindow.Html.Contains("banned");
                        if (modalWindow.Html.Contains("banned"))
                        {
                            WCFClient.Instance.GetPipeProxy.RemoteLog("[" + ESCache.Instance.CharName + "]" + modalWindow.Html);
                        }

                        disableInstance |= modalWindow.Html.ToLower().Contains("The authentication token provided by the launcher".ToLower());
                        pause |= modalWindow.Html.Contains("Cannot move");
                        ...
                        if (modalWindow.Guid == "form.MessageBox" && modalWindow.IsDialog && modalWindow.IsModal && modalWindow.IsKillable)
                        {
                            ...
                            {
                                sayYes |= modalWindow.Html.Contains(
                        ...
                        {
                            sayYes |= (modalWindow.Html.Contains("item will be moved to") &&
                                        modalWindow.Html.Contains("Do you wish to proceed")
                                        );
                            sayYes |= (modalWindow.Html.Contains("directly injected to") &&
                                        modalWindow.Html.Contains("Do you wish to proceed")
                                        );

                            sayYes |= (modalWindow.Html.Contains("The following items will be redeemed") &&
                                        modalWindow.Html.Contains("Do you wish to proceed")
                                        );
                        }

                        sayNo |= modalWindow.Html.Contains("priced well below the average:"); //selling items at a price that is too low!
                        close |= modalWindow.Html.Contains("Do you really want to quit now?");
                        close |= modalWindow.Html.Contains("Please make sure your characters are out of harm");
                        close |= modalWindow.Html.Contains("the servers are down for 30 minutes each day for maintenance and updates");
                        close |= modalWindow.Html.Contains("Item cannot be moved back to a loot container.");
                        close |= modalWindow.Html.Contains("you do not have the cargo space");
                        close |= modalWindow.Html.Contains("You are too far away from the acceleration gate to activate it!");
                        close |= modalWindow.Html.Contains("maximum distance is 2500 meters");
                        close |= modalWindow.Html.Contains("Broker found no match for your order");
                        close |= modalWindow.Html.Contains("All the weapons in this group are already full");
                        //close |=
                        //    modalWindow.Html.Contains(
                        //        "If you decline of fail a mission from an agent he/she might become displeased and lower your standing towards him/her. You can decline a mission every four hours without penalty");
                        close |= modalWindow.Html.Contains("Do you wish to proceed with this dangerous action?");
                        close |= modalWindow.Html.Contains("weapons in that group are already full");
                        close |= modalWindow.Html.Contains("No rigs were added to or removed from the ship");
                        close |= modalWindow.Html.Contains("You can't fly your active ship into someone else's hangar");
                        close |= modalWindow.Html.Contains("You can't do this quite so fast");
                        clearPocket |= modalWindow.Html.Contains("This gate is locked!");

                        close |= modalWindow.Html.Contains("The Zbikoki's Hacker Card");
                        close |= modalWindow.Html.Contains(" units free.");
                        close |= modalWindow.Html.Contains("already full");
                        close |= modalWindow.Html.Contains("All the weapons in this group are already full");
                        close |=
                            modalWindow.Html.Contains(
                                "At any time you can log in to the account management page and change your trial account to a paying account");
                        close |= modalWindow.Html.ToLower().Contains("please make sure your characters are out of harms way");
                        restart |= modalWindow.Html.ToLower().Contains("accepting connections");
                        restart |= modalWindow.Html.ToLower().Contains("could not connect") && !modalWindow.Html.ToLower().Contains("to chat server");
                        restart |= modalWindow.Html.ToLower().Contains("the connection to the server was closed");
                        restart |= modalWindow.Html.ToLower().Contains("server was closed");
                        close |= modalWindow.Html.ToLower().Contains("make sure your characters are out of harm");
                        restart |= modalWindow.Html.ToLower().Contains("connection to server lost");
                        restart |= modalWindow.Html.ToLower().Contains("the socket was closed");
                        restart |= modalWindow.Html.ToLower().Contains("the specified proxy or server node");
                        close |= modalWindow.Html.ToLower().Contains("starting up");
                        restart |= modalWindow.Html.ToLower().Contains("unable to connect to the selected server");
                        restart |= modalWindow.Html.ToLower().Contains("could not connect to the specified address");
                        restart |= modalWindow.Html.ToLower().Contains("connection timeout");
                        restart |= modalWindow.Html.ToLower().Contains("the cluster is not currently accepting connections");
                        close |= modalWindow.Html.ToLower().Contains("your character is located within");
                        close |= modalWindow.Html.ToLower().Contains("the transport has not yet been connected");
                        close |= modalWindow.Html.ToLower().Contains("the user's connection has been usurped");
                        close |= modalWindow.Html.ToLower().Contains("the EVE cluster has reached its maximum user limit");
                        restart |= modalWindow.Html.ToLower().Contains("the connection to the server was closed");
                        close |= modalWindow.Html.ToLower().Contains("client is already connecting to the server");
                        close |= modalWindow.Html.ToLower().Contains("client update is available and will now be installed");
                        close |= modalWindow.Html.ToLower().Contains("change your trial account to a paying account");
                        close |= modalWindow.Html.ToLower().Contains("Not all the items could be fitted");

                        restart |= modalWindow.Html.Contains("The user's connection has been usurped on the proxy");
                        restart |= modalWindow.Html.Contains("The connection to the server was closed");
                        restart |= modalWindow.Html.Contains("server was closed");
                        restart |= modalWindow.Html.Contains("The socket was closed");
                        restart |= modalWindow.Html.Contains("The connection was closed");
                        restart |= modalWindow.Html.Contains("Connection to server lost");
                        restart |= modalWindow.Html.Contains("The user connection has been usurped on the proxy");
                        restart |= modalWindow.Html.Contains("The transport has not yet been connected, or authentication was not successful");
                        restart |= modalWindow.Html.Contains("Your client has waited");
                        restart |= modalWindow.Html.Contains("This could mean the server is very loaded");
                        restart |= modalWindow.Html.Contains("Local cache is corrupt");
                        restart |= modalWindow.Html.Contains("Local session information is corrupt");
                        restart |= modalWindow.Html.ToLower().Contains("you are already performing a".ToLower());
                        restart |= modalWindow.Html.ToLower().Contains("the socket was closed".ToLower());
                        restart |= modalWindow.Html.ToLower().Contains("the connection was closed".ToLower());
                        restart |= modalWindow.Html.ToLower().Contains("connection to server lost.".ToLower());
                        restart |= modalWindow.Html.ToLower().Contains("local cache is corrupt".ToLower());
                        restart |= modalWindow.Html.ToLower().Contains("the client's local session".ToLower());
                        restart |= modalWindow.Html.ToLower().Contains("restart the client prior to logging in".ToLower());

                        quit |= modalWindow.Html.ToLower().Contains("the cluster is shutting down".ToLower());

                        sayYes |= modalWindow.Html.Contains("eject from the ship?");
                        sayYes |= modalWindow.Html.Contains("objectives requiring a total capacity");
                        sayYes |= modalWindow.Html.Contains("your ship only has space for");
                        sayYes |= modalWindow.Html.Contains("Are you sure you want to remove location");
                        sayYes |= modalWindow.Html.Contains("Are you sure you would like to decline this mission");
                        sayYes |= modalWindow.Html.Contains("has no other missions to offer right now. Are you sure you want to decline");
                        sayYes |= modalWindow.Html.Contains("You are about to remove a storyline mission from your journal");
                        sayYes |= modalWindow.Html.Contains("If you quit this mission you will lose standings with your agent");
                        sayYes |= modalWindow.Html.ToLower().Contains("could not connect to chat server".ToLower());

                        sayOk |= modalWindow.Html.Contains("Warning! This star system has been secured by EDENCOM forces"); //this window isnt a modalWindow for some odd reason? FixMe
                        sayOk |= modalWindow.Html.Contains("Are you sure you want to accept this offer?");
                        sayOk |= modalWindow.Html.Contains("You do not have an outstanding invitation to this fleet.");
                        sayOk |= modalWindow.Html.Contains("You have already selected a character for this session.");
                        sayOk |= modalWindow.Html.Contains("The transport has not yet been connected, or authentication was not successful");
                        sayOk |= modalWindow.Html.ToLower().Contains("local session information is corrupt");
                        ...
                        needHumanIntervention |= modalWindow.Html.Contains("One or more mission objectives have not been completed");
                        needHumanIntervention |= modalWindow.Html.Contains("Please check your mission journal for further information");
                        sayOk |= modalWindow.Html.Contains("You have to be at the drop off location to deliver the items in person");
                        needHumanIntervention |= modalWindow.Html.Contains("cargo units would be required to complete this operation.");

                        stackHangars |= modalWindow.Html.Contains("as there are simply too many items here already");
                    }
                ...
                if (restart)
                {
                    ...
                    Log.WriteLine(msg);
                    WCFClient.Instance.GetPipeProxy.RemoteLog(msg);
                    ESCache.Instance.CloseEveReason = "restart";
