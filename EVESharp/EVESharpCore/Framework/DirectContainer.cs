...
namespace EVESharpCore.Framework
{
    ...
    public class DirectContainer : DirectInvType
    {
        ...
        ///     Flag reference
        /// </summary>
        private readonly PyObject _pyFlag;

        /// <summary>
        ///     Inventory reference
        /// </summary>
        private readonly PyObject _pyInventory;

        /// <summary>
        ///     Is this the ship's modules 'container'
        /// </summary>
        private readonly bool _shipModules;
        ...
        public bool IsReady
        {
            get
            {
                ...
                PyObject listed = _pyInventory.Attribute("listed");
        ...
        public List<DirectItem> Items
        {
            get
            {
                if (_items == null)
                {
                    ...
                    // Special case
                    int categoryShip = (int)DirectEve.Const.CategoryShip;
        ...
        public int UsedCapacityPercentage
        {
            get
            {
                ...
                return (int)Math.Round(UsedCapacity / Capacity, 0);
            }
        }
        ...
        public bool Add(IEnumerable<DirectItem> items)
        {
            ...
            long fromCointainerId = items.First().LocationId;
        ...
        public bool Jettison(IEnumerable<long> itemIds)
        {
            ...
            PyObject jettison = DirectEve.GetLocalSvc("gameui").Call("GetShipAccess").Attribute("Jettison");
        ...
        internal static DirectContainer GetCorporationHangar(DirectEve directEve, string divisionName)
        {
            ...
            PyObject divisions = directEve.GetLocalSvc("corp").Call("GetDivisionNames");
            for (int i = 1; i <= 7; i++)
        ...
        internal static DirectContainer GetItemHangar(DirectEve directEve)
        {
            ...
            bool inStructure = directEve.Session.Structureid.HasValue;
            string name = inStructure ? "StructureItemHangar" : "StationItems";
            long id = inStructure ? directEve.Session.Structureid.Value : (long)directEve.Const.ContainerHangar;
            PyObject inventory = GetInventory(directEve, "GetInventory", id);
        ...
        internal static DirectContainer GetShipHangar(DirectEve directEve)
        {
            ...
            bool inStructure = directEve.Session.Structureid.HasValue;
            string name = inStructure ? "StructureShipHangar" : "StationShips";
            long id = inStructure ? directEve.Session.Structureid.Value : (long)directEve.Const.ContainerHangar;
            PyObject inventory = GetInventory(directEve, "GetInventory", id);
            return new DirectContainer(directEve, inventory, directEve.Const.FlagHangar, name);
        }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static PyObject GetInventory(DirectEve directEve, string method, long id)
        {
            Dictionary<PyObject, PyObject> inventories = directEve.GetLocalSvc("invCache").Attribute("inventories").ToDictionary();
            foreach (KeyValuePair<PyObject, PyObject> inventory in inventories)
            {
                //directEve.Log(inventory.Key.LogObject());
                //directEve.Log(inventory.Value.LogObject());
                long keyid = (long)inventory.Key.GetItemAt(0);
