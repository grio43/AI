                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            int iCount = 0;
                                            if (DirectEve.Interval(10000)) Log.WriteLine("LogAndMessagesWindow: Found [" + pyListOfMessageEntries.Count + "] entries");
                                            foreach (var pyMessageEntry in pyListOfMessageEntries)
                                            {
                                                iCount++;
                                                if (pyMessageEntry.Attribute("name").IsValid && pyMessageEntry.Attribute("name").ToUnicodeString().ToLower().Contains("entry_".ToLower()))
                                                {
                                                    //.children._childrenObjects[0] - __content
                                                    ...
                                                    if (pyMessage.Attribute("name").IsValid && pyMessage.Attribute("name").ToUnicodeString().ToLower() == "entryLabel".ToLower())
                                                    {
                                                        ...
                                                        {
                                                            if (DirectEve.Interval(10000)) Logging.Log.WriteLine("LogAndMessagesWindow: [" + iCount + "][" + pyMessage.Attribute("text").ToUnicodeString() + "]");
                                                        }
                                                        else
                                                        {
                                                            if (DirectEve.Interval(10000)) Logging.Log.WriteLine("LogAndMessagesWindow: [" + iCount + "][text not found]");
                                                        }
                                                    }
                                                }
                                            }
