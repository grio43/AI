...
namespace EVESharpCore.Framework
{
    ...
    public class DirectMe : DirectObject
    {
        ...
        public bool CanIWarp()
        {
            ...
            // siege .. if you have a seive module detect its active
            // anything else?
      ...
      : 0;
        ...
        public float AbyssalRemainingSeconds => AbyssalTimerExist
            ? DirectEve.GetLocalSvc("infoPanel").Attribute("combatTimerContainer").Attribute("abyssalContentExpirationTimer").Attribute("ratio").ToFloat() * 20 * 60
            : 0;

        public float VoidSpaceRemainingSeconds => VoidSpaceTimerExist
            ? DirectEve.GetLocalSvc("infoPanel").Attribute("combatTimerContainer").Attribute("voidSpaceContentExpirationTimer").Attribute("ratio").ToFloat() * 20 * 60
            : 0;
        ...
        public float? Wealth
        {
            get
            {
                try
                {
                    //if (DirectEve.GetLocalSvc("wallet").HasAttrString("wealth"))
                    //{
