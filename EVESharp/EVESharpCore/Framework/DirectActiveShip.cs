    ...
    public class DirectActiveShip : DirectItem
    {
        ...
        public void AdjustSpeedIfGoingTooFast(double myVelocity)
        {
            try
            {
                ...
                int _badGuessAtMaxSpeed = 400;
                if (DirectEve.Modules.Any(i => i.IsMicroWarpDrive && i.IsActive))
                {
                    if (ESCache.Instance.ActiveShip.IsBattleship)
                    {
                        _badGuessAtMaxSpeed = 1000;
                    }
                    else if (ESCache.Instance.ActiveShip.IsCruiser)
                    {
                        _badGuessAtMaxSpeed = 2000;
                    }
                    else if (ESCache.Instance.ActiveShip.IsFrigate)
                    {
                        _badGuessAtMaxSpeed = 3000;
                    }
                    else if (ESCache.Instance.ActiveShip.IsAssaultShip)
                    {
                        _badGuessAtMaxSpeed = 3000;
                    }
                }

                if (DirectEve.Modules.Any(i => i.IsAfterburner && !i.IsMicroWarpDrive && i.IsActive))
                {
                    if (ESCache.Instance.ActiveShip.IsBattleship)
                    {
                        _badGuessAtMaxSpeed = 500;
                    }
                    else if (ESCache.Instance.ActiveShip.IsCruiser)
                    {
                        _badGuessAtMaxSpeed = 800;
                    }
                    else if (ESCache.Instance.ActiveShip.IsAssaultShip)
                    {
                        _badGuessAtMaxSpeed = 900;
                    }
                    else if (ESCache.Instance.ActiveShip.IsFrigate)
                    {
                        _badGuessAtMaxSpeed = 1200;
                    }
                }

                if (DirectEve.Session.IsAbyssalDeadspace)
                {
                    if (DirectEve.ActiveShip.Entity.IsLocatedWithinSpeedCloud || DirectEve.ActiveShip.Entity.IsTooCloseToSpeedCloud)
                    {
                        try
                        {
        ...
        public bool IsShipWithFleetHangar
        {
            get
            {
                try
                {
                    if (GroupId == (int)Group.TransportShip || //TransportShip
                        GroupId == (int)Group.Carrier || //Carrier
                        GroupId == (int)Group.Dreadnaught || //Dreadnaught
        ...
        public void ToggleShipConfigOption(ShipConfigOption option)
        {
            var shipConfigSvc = DirectEve.GetLocalSvc("shipConfig");
