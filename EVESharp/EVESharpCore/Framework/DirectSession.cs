    ...
    public class DirectSession : DirectObject
    {
        ...
        public bool IsVoidSpace
        {
            get
            {
                ...
                if (DirectEve.Entities.Any(i => i.IsStargate))
                {
                    //if (DirectEve.Interval(30000, 30000, ItemId.ToString())) ESCache.Instance.TaskSetEveAccountAttribute(nameof(EveAccount.IsInAbyss), false);
                    return false;
                }
        ...
        public bool IsWspace
        {
            get
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (DirectEve.Entities.Any(x => x.IsPlanet) && !DirectEve.Entities.All(x => x.IsStargate))
                        return false;
        ...
        public DirectSolarSystem SolarSystem
        {
            get
            {
                if (SolarSystemId != null && !ESCache.Instance.DirectEve.Session.IsWspace)
        ...
        public string SolarSystemName
        {
            get
            {
                if (SolarSystemId != null && !ESCache.Instance.DirectEve.Session.IsWspace && SolarSystem != null)
