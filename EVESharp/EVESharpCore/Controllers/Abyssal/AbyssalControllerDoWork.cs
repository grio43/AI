    ...
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        private bool ShouldWeGoIdle()
        {
            ...
            if (ESCache.Instance.ActiveShip != null)
            {
                ...
                {
                    if (AbyssalFilamentsActivated % 4 != 0)
                    {
                        if (DirectEve.Interval(30000)) Log("AbyssalFilamentsActivated[" + AbyssalFilamentsActivated + "] is not evenly divisible by 4 return false");
                        return false;
                    }
                }
            }
        ...
        public override void DoWork()
        {
            ...
            if (ESCache.Instance.InStation && ESCache.Instance.DeactivateScheduleAndCloseAfterNextDock)
            {
                ...
                if (ESCache.Instance.EveAccount.UseScheduler)
                {
                    ...
                    ESCache.Instance.DeactivateScheduleAndCloseAfterNextDock = false;
                    return;
                }
        ...
        internal void AbyssalEveryPulse()
        {
            ...
            if (DirectEve.Interval(4000, 5000))
            {
                ...
                if (ESCache.Instance.EveAccount.AbyssStage != (int)CurrentAbyssalStage)
                    WCFClient.Instance.GetPipeProxy.SetEveAccountAttributeValue(ESCache.Instance.CharName, nameof(EveAccount.AbyssStage), (int)CurrentAbyssalStage);
            }
        ...
        internal void AbyssalStart()
        {
            ...
            if (ESCache.Instance.InSpace && fbmx.DistanceTo(ESCache.Instance.DirectEve.ActiveShip.Entity) <= 15000 && !AbyssalNeedRepair(true) && !IsAnyOtherNonFleetPlayerOnGrid)
            {
                ...
                if (!ESCache.Instance.DirectEve.Modules.All(m => m.HeatDamagePercent == 0)
                    ...
                    && ESCache.Instance.CurrentShipsCargo.Items.Any(i => i.TypeId == _naniteRepairPasteTypeId && i.Stacksize > 30)
                    && ESCache.Instance.InAbyssalDeadspace)
                {
                    RepairModules();
            ...
            if (ESCache.Instance.InSpace && !AbyssalNeedRepair())
            {
                ...
                if (fbmx.DistanceTo(ESCache.Instance.DirectEve.ActiveShip.Entity) <= 15000)
                {
        ...
        internal bool CheckDroneBayForAllDronesNeeded()
        {
            ...
            if (_droneBayItemList != null && _droneBayItemList.Any())
            {
                foreach (var t in _droneBayItemList) // .RandomPermutation()) // change the order
                {
                    ...
                    int avaiableInHangar = 0;
                    if (ESCache.Instance.InStation)
                    {
                        avaiableInHangar = itemHangar.Items.Where(d => d.TypeId == typeId).Sum(d => d.Stacksize);
                    }
                    ...
                    if (ESCache.Instance.InStation)
                    {
                        if (missingInDroneBay > avaiableInHangar)
                        {
                            ...
                            Log($"Error: OutOfDrones: Not enough drones left available. [{typeId}][{missingItem.TypeName}] AvaiableInHangar [{avaiableInHangar}] missingInDroneBay [{missingInDroneBay}]");
                    ...
                    else if (ESCache.Instance.InSpace)
                    {
                        ...
                        if (shipsCargo != null && shipsCargo.Items.Any())
                        {
                            if (DebugConfig.DebugArm)
                            {
                                ...
                                Log("shipsCargoBayList to Load [" + _shipsCargoBayList.Count() + "]");
                                foreach (var shipsCargoBayItem in _shipsCargoBayList.OrderBy(i => i.Item1))
                                {
                                    intCargoBayList++;
                                    Log("[" + intCargoBayList + "] TypeID [" + shipsCargoBayItem.Item1 + "] Quantity[" + shipsCargoBayItem.Item2 + "]");
                                }

                                int intCargoHoldItems = 0;
                                Log("CargoHold Items Found [" + shipsCargo.Items.Count() + "]");
                                foreach (var cargoHoldItem in shipsCargo.Items.OrderBy(i => i.TypeId))
                                {
                                    intCargoHoldItems++;
                                    Log("[" + intCargoHoldItems + "][" + cargoHoldItem.TypeName + "] TypeID [" + cargoHoldItem.TypeId + "] Quantity [" + Math.Abs(cargoHoldItem.Quantity) + "]");
                                }
                            }
                        }
        ...
        internal void AbyssalArm()
        {
            try
            {
                ...
                //{
                //Log("AbyssalArm: if (DirectEve.Interval(60000)) Util.FlushMemIfThisProcessIsUsingTooMuchMemory(1024);");
                //Util.FlushMemIfThisProcessIsUsingTooMuchMemory(1024);
                //return;
                //}
                ...
                if (!CheckDroneBayForAllDronesNeeded()) return;
                ...
                if (ESCache.Instance.ActiveShip.HasDroneBay && !droneBay.Items.Any())
                {
                    // verify drone bay has the correct amount of drones loaded
        ...
        internal List<DirectBookmark> ListOfFilamentBookmarks
        {
            get
            {
                ...
                {
                    _listOfFilamentBookmarks = ESCache.Instance.DirectEve.Bookmarks.Where(b => b.Title.ToLower().Contains(_filamentSpotBookmarkName.ToLower()))
        ...
        internal void AbyssalTravelToFilamentSpot()
        {
            ...
            if (AbyssalNeedRepair())
            {
                if (DirectEve.Interval(15000, 25000))
                {
                    foreach (DirectUIModule mod in DirectEve.Modules.Where(m => m.HeatDamagePercent > 1))
                    {
                ...
                Log("Damaged Modules Found! Go repair.");
                myAbyssalState = AbyssalState.TravelToRepairLocation;
                return;
            }
            ...
            if (ESCache.Instance.Stations.Any(i => i.IsOnGridWithMe))
            {
                if (!CheckCargoHoldForAllSuppliesNeeded()) return;
                if (!CheckDroneBayForAllDronesNeeded()) return;
            }
