    ...
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        public bool NeedToDumpDatabaseSurveys()
        {
            try
            {
                ...
                if (!DirectEve.Interval(60000 * 15)) //15min
                    return false;
        ...
        internal void DumpDatabaseSurveys()
        {
            switch (_dataSurveyDumpState)
            {
                ...
                case DataSurveyDumpState.EmptyTransportShip:
                    EmptyTransportShip();
                    break;
                case DataSurveyDumpState.LoadSurveysAndLootToCargo:
                    LoadSurveysAndOrLootToCargo(true, true);
                    break;
                ...
                case DataSurveyDumpState.LoadLootToCargo:
                    LoadSurveysAndOrLootToCargo(false, true);
                    break;
        ...
        private void EmptyTransportShip()
        {
            ...
            {
                _dataSurveyDumpState = DataSurveyDumpState.LoadSurveysAndLootToCargo;
            }
        ...
        private void LoadSurveysAndOrLootToCargo(bool boolLoadSurveys = true, bool boolLoadLoot = true)
        {
            ...
            if (ESCache.Instance.CurrentShipsCargo.Items.Any(e => e.TypeId == (int)TypeID.TriglavianSurveyDatabase))
            {
                ...
                if (_dataSurveyDumpState == DataSurveyDumpState.LoadSurveysAndLootToCargo)
                {
            ...
            Log("No Survey Databases Found in Cargo");
        ...
        private void DumpSurveys()
        {
            ...
            var loot2dump = ESCache.Instance.ItemHangar.Items.Where(i =>i.TypeId != _filamentTypeId && i.IsAbyssalLootToSell && !i.IsBlueprintCopy).ToList();
            if (loot2dump.Count > 10)
            {
                loot2dump = loot2dump.RandomPermutation().OrderByDescending(x => x.TypeId == (int)TypeID.TriglavianSurveyDatabase)
                                                         .ThenByDescending(x => x.CategoryId == (int)CategoryID.Skill)
                                                         .ThenByDescending(x => x.TypeName.Contains("Zero-Point Condensate"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Crystalline Isogen-10"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Cataclysmic"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Chaotic"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Raging"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Fierce"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Agitated"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Calm"))
                                                         .ThenByDescending(x => x.TypeName.Contains("Tranquil"))
                                                         .Take(4).ToList(); //sort filaments last
            }
