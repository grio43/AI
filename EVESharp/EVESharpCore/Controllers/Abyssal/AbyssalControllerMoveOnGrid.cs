...
namespace EVESharpCore.Controllers.Abyssal
{
    ...
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        private bool WeHavePotentialCombatTargetsThatArentReadyToShoot
        {
            get
            {
                    ...
                    ESCache.Instance.ActiveShip.IsFrigOrDestroyerWithDroneBonuses)
                {
                    ...
                    if (DirectEve.Interval(5000)) Log("WeHavePotentialCombatTargetsThatArentReadyToShoot: All drones must not be attacking!");
                    return true;
                }
        ...
        private bool boolSpeedTank_IsItSafeToMoveToMTUOrLootWrecks
        {
            get
            {
                if (ESCache.Instance.ActiveShip.IsShieldTanked && 50 > ESCache.Instance.ActiveShip.ShieldPercentage)
                {
                    if (DebugConfig.DebugNavigateOnGrid) Log("50 > S[" + Math.Round(ESCache.Instance.ActiveShip.ShieldPercentage, 2) + "%] return false");
                    return false;
                }

                if (ESCache.Instance.ActiveShip.IsArmorTanked && 50 > ESCache.Instance.ActiveShip.ArmorPercentage)
                {
                    if (DebugConfig.DebugNavigateOnGrid) Log("50 > A[" + Math.Round(ESCache.Instance.ActiveShip.ArmorPercentage, 2) + "%] return false");
                    return false;
                }
                ...
                if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.EphialtesCruiserSpawn)
                {
                    ...
                    {
                        if (DebugConfig.DebugNavigateOnGrid) Log("if (4 >= Combat.PotentialCombatTargets.Count(i => i.IsNPCCruiser)) return true");
                        return true;
                    }
        ...
        private bool IsItSafeToMoveToMTUOrLootWrecks
        {
            get
            {
                ...
                if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.AbyssalOvermindDroneBSSpawn)
                {
                    if (_droneBattleshipsOnGridCount > _droneBattleshipsTankThreshold)
                    {
                        if (DebugConfig.DebugNavigateOnGrid) Log("_droneBattleshipsOnGridCount [" + _droneBattleshipsOnGridCount + "] > _droneBattleshipsTankThreshold [" + _droneBattleshipsTankThreshold + "] return false");
                        return false;
                    }
        ...
        private bool IsSituationThatNecessitatesWeUseOurUniqueOrbitRoutine
        {
            get
            {
                ...
                if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.AbyssalOvermindDroneBSSpawn && (ESCache.Instance.ActiveShip.Entity.TypeId == (int)TypeID.Gila || ESCache.Instance.ActiveShip.TypeId == (int)TypeID.StormBringer))
                {
                    if (DebugConfig.DebugNavigateOnGrid) Log("DetectSpawn [" + AbyssalSpawn.DetectSpawn + "]");
                    if (Combat.PotentialCombatTargets.Any(i => i.IsNPCBattleship))
                    {
            ...
            if (!IsKarenHuggingSuppressor)
            {
                if (boolShouldWeBeSpeedTanking && ESCache.Instance.ActiveShip.Entity.IsTooCloseToSmallDeviantAutomataSuppressor && ESCache.Instance.PlayerSpawnLocation.DistanceTo(ESCache.Instance.AbyssalDeadspaceSmallDeviantAutomataSuppressor) > 20000 && !WeAreRetrievingAnMTUThatIsFarAway)
                {
                    ...
                    if (boolShouldWeBeSpeedTanking && ESCache.Instance.Weapons.Any(i => i.IsMissileLauncher && !i._module.IsVortonProjector) && ESCache.Instance.ActiveShip.TypeId != (int)TypeID.Gila && !ESCache.Instance.ActiveShip.IsFrigOrDestroyerWithDroneBonuses)
                    {
                        ...
                        {
                            if (Combat.PotentialCombatTargets.Any(i => !i.IsCruiser))
                            {
                                if (!ESCache.Instance.PlayerSpawnLocation._directEntity.IsTooCloseToSmallDeviantAutomataSuppressor && Combat.PotentialCombatTargets.Any(i => i.IsReadyToShoot))
                                {
                                    ESCache.Instance.PlayerSpawnLocation.Orbit(15000);
        ...
        internal bool SpeedTank_ConcordSpawn()
        {
            ...
            if (Combat.PotentialCombatTargets.Any(i => i.IsNPCFrigate || i.IsNPCDestroyer))
            {
                if (Combat.PotentialCombatTargets.Count(i => i.IsNPCFrigate || i.IsNPCDestroyer) >= 3 || Combat.PotentialCombatTargets.Any(i => (i.IsNPCFrigate || i.IsNPCDestroyer) && !i.IsReadyToShoot))
                {
                    if (DebugConfig.DebugNavigateOnGrid) Log("if (Combat.PotentialCombatTargets.Any(i => i.IsNPCFrigate || i.IsNPCDestroyer))");
                    if (51000 > Combat.PotentialCombatTargets.Where(i => i.IsNPCDestroyer || i.IsNPCFrigate).OrderBy(i => i.Distance).FirstOrDefault().Distance)
                    {
                        if (DebugConfig.DebugNavigateOnGrid) Log("if (31000 > Combat.PotentialCombatTargets.Where(i => i.IsNPCDestroyer || i.IsNPCFrigate).OrderBy(i => i.Distance).FirstOrDefault().Distance)");
                        if (ESCache.Instance.ActiveShip.IsSpeedTankedShortRangeShip)
                        {
                            Combat.PotentialCombatTargets.Where(i => i.IsNPCDestroyer || i.IsNPCFrigate).OrderBy(i => i.Distance).FirstOrDefault().Orbit(9000);
                            return true;
                        }
                        ...
                        Combat.PotentialCombatTargets.Where(i => i.IsNPCDestroyer || i.IsNPCFrigate).OrderBy(i => i.Distance).FirstOrDefault().Orbit(orbitDistanceToUse); //, false, "IsNPCFrigate x 4 are too close: they are inside 24k");
                        return true;
                    }
                }
                ...
                if (ESCache.Instance.ActiveShip.IsSpeedTankedShortRangeShip)
                {
                    ...
                    Combat.PotentialCombatTargets.Where(i => i.IsNPCFrigate || i.IsNPCDestroyer).OrderBy(i => i.Distance).FirstOrDefault().Orbit(9000);
                    return true;
                }
                ...
                if (ESCache.Instance.ActiveShip.IsFrigOrDestroyerWithDroneBonuses)
                {
                    ...
                    Combat.PotentialCombatTargets.Where(i => i.IsNPCFrigate || i.IsNPCDestroyer).OrderBy(i => i.Distance).FirstOrDefault().Orbit(orbitDistanceToUse);
                    return true;
                }
            }
        ...
        internal bool SpeedTank_KarybdisTyrannosSpawn()
        {
            if (Combat.PotentialCombatTargets.Any(i => i.IsNPCCruiser) && (!ESCache.Instance.ActiveShip.IsFrigate))
            {
                ...
                ESCache.Instance.AbyssalCenter._directEntity.DirectAbsolutePosition.OrbitWithHighTransversal(13000);
                return true;
            }
            ...
            if (Combat.PotentialCombatTargets.Any())
            {
                ...
                if (closest != null)
                {
                    ...
                    if (25000 > closest.Distance)
                    {
                        ...
                        closest._directEntity.DirectAbsolutePosition.OrbitWithHighTransversal((int)Combat.MaxRange - 5000); //, false, "IsNPCFrigate x 4 are too close: they are inside 24k");
                        return true;
                    }
                }
        ...
        **/
        ...
        internal bool MoveToOverride
        {
            get
            {
                try
                {
                    ...
                    {
                        if (DirectEve.Interval(5000))
                            Log($"Keep Transversal Up! IsSituationThatNecessitatesWeUseOurUniqueOrbitRoutine: True");
                        ...
                        if (DirectEve.HasFrameChanged())
                        {
                            ...
                            if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.KarybdisTyrannosSpawn)
                            {
                                ...
                                if (Combat.PotentialCombatTargets.Any(i => i._directEntity.IsAbyssalKaren))
                                {
                                    ...
                                    if (WeNeedToGetIntoRange && Combat.PotentialCombatTargets.FirstOrDefault(i => i._directEntity.IsAbyssalKaren).Distance > rangeNeededForKaren)
                                    {
                                        if (35000 > ESCache.Instance.AbyssalCenter.Distance || 40000 > Combat.PotentialCombatTargets.FirstOrDefault(i => i._directEntity.IsAbyssalKaren).Distance) //if we arent this close to the abyssalcenter wait until we are before getting closer to karen
                                        {
                                            if (30000 > Combat.PotentialCombatTargets.FirstOrDefault(i => i._directEntity.IsAbyssalKaren).Distance)
                            ...
                            if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.AbyssalOvermindDroneBSSpawn)
                            {
                                ...
                                if (Combat.PotentialCombatTargets.Any(i => i._directEntity.IsAbyssalDroneBattleship))
                                {
                                    ...
                                    if (WeNeedToGetIntoRange && Combat.PotentialCombatTargets.FirstOrDefault(i => i._directEntity.IsAbyssalDroneBattleship).Distance > rangeNeededForKaren)
                                    {
                            ...
                            Log("default orbit: 10k around moveToTargetEntity [" + moveToTargetEntity.TypeName + "]");
                            _moveToTarget_MTU_Wreck_Can_or_Gate_Entity.Orbit(10000);
                        }
        ...
        public List<EntityCache> GetSortedTargetList(IEnumerable<EntityCache> list, IEnumerable<EntityCache> averageDistEntities = null)
        {
            return list
                    ...
                    .ThenBy(e => averageDistEntities == null || !averageDistEntities.Any()
                           ? (int)(e.Distance / 5000)
                           : (int)e.DistanceTo(averageDistEntities) / 5000)
                    .ToList();
        }
                        ...
                        _lastMTULaunch.AddSeconds(12) < DateTime.UtcNow)
                    {
                        ...
                        if (cont.Items.Any())
                        {
                            ...
                            if (DirectEve.Interval(2500, 3500, freeCargo.ToString()) && DirectEve.GetShipsCargo().Add(cont.Items))
                            {
                                ...
                                WCFClient.Instance.GetPipeProxy.SetEveAccountAttributeValue(ESCache.Instance.EveAccount.GUID, nameof(EveAccount.LootValueGatheredToday), Math.Round(ESCache.Instance.EveAccount.LootValueGatheredToday + _valueLooted, 0));
