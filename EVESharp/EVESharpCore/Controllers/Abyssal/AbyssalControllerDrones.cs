    ...
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        private bool ReturnDrones(List<AbyssalDrone> dronesIWant)
        {
            ...
            if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.LuciferSpawn)
            {
                ...
                if (_droneRecallsStage >= _droneRecallLimit)
                {
        ...
        public bool HandleDroneRecallAndDeployment()
        {
            ...
            if (!Combat.PotentialCombatTargets.Any() || Combat.PotentialCombatTargets.All(i => i.IsAbyssalDeadspaceTriglavianExtractionNode))
            {
                var droneAvgDist = allDronesInSpace.Any() ? allDronesInSpace.Average(e => e.Distance) : 0;
                //var minDroneSpeed = 2000d;
            ...
            {
                if (Combat.PotentialCombatTargets.Any() && Combat.PotentialCombatTargets.OrderBy(i => i._directEntity.AbyssalTargetPriority).FirstOrDefault()._directEntity.IsTargetingDrones)
                {
        ...
        private bool ManageDrones()
        {
            ...
            if (ThisSpawnEatsDrones && _droneRecallsStage > 4)
            {
                if (allDronesInSpace.Any())
                {
                    if (RecallDrones())
        ...
        private bool SplitDronesIntoTwoGroups(EntityCache target1, EntityCache target2, List<EntityCache> dronesToUse)
        {
            ...
            if (ESCache.Instance.ActiveShip.IsCruiser)
            {
                if (ESCache.Instance.ActiveShip.Entity.TypeId != (int)TypeID.Ishtar &&
                    ESCache.Instance.ActiveShip.Entity.TypeId != (int)TypeID.Gila)
                {
            ...
            if (t2.Any(e => e.TypeName.ToLower().Contains("kikimora")))
            {
        ...
        private bool DronesEngageTargets()
        {
            ...
            if (DirectEve.Interval(800, 1200) && ESCache.Instance.Targets.Where(e => e.GroupId != (int)Group.AssaultShip && !e.IsAbyssalDeadspaceTriglavianBioAdaptiveCache && !e.IsAbyssalDeadspaceTriglavianExtractionNode && !e.IsWreck).Any())
            {
                ...
                //
                var target1 = GetSortedTargetList(ESCache.Instance.Targets.Where(e => e.GroupId != (int)Group.AssaultShip && e.Distance < ESCache.Instance.ActiveShip.GetDroneControlRange() && !e.IsAbyssalDeadspaceTriglavianExtractionNode && !e.IsWreck), group1).OrderByDescending(x => !x.IsAbyssalDeadspaceTriglavianBioAdaptiveCache).Take(1).FirstOrDefault();
                var target2 = GetSortedTargetList(ESCache.Instance.Targets.Where(e => e.GroupId != (int)Group.AssaultShip && e.Distance < ESCache.Instance.ActiveShip.GetDroneControlRange() && !e.IsAbyssalDeadspaceTriglavianExtractionNode && !e.IsWreck), group2).OrderByDescending(x => !x.IsAbyssalDeadspaceTriglavianBioAdaptiveCache).Skip(1).Take(1).FirstOrDefault();
                ...
                else
                {
                    ...
                    //
                    var target = GetSortedTargetList(ESCache.Instance.Targets.Where(e => e.GroupId != (int)Group.AssaultShip && e.Distance < ESCache.Instance.ActiveShip.GetDroneControlRange() && !e.IsAbyssalDeadspaceTriglavianBioAdaptiveCache && !e.IsAbyssalDeadspaceTriglavianExtractionNode && !e.IsWreck), allDronesInSpace).FirstOrDefault();
                    if (target != null)
                    {
                        if (DebugConfig.DebugDrones)
                            Log("if (target != null)");
                        ...
                        if (ds.Any()) // is there any drone not attacking our current target
                        {
                            ...
                            if (target._directEntity.EngageTargetWithDrones(ds.Select(d => d.Id).ToList()))
                            {
                                ...
                                // only engage with non returning drones
                                _droneEngageCount++;
                                Log($"Engaging [{ds.Count()}] drones on [{target.TypeName}][{Math.Round(target.Distance / 1000, 0)}][{target.Id}]"); //Priority [{target.AbyssalTargetPriority}]");
        ...
        private bool IgnoreTargetSwap(DirectEntity oldTarget, EntityCache newTarget, List<EntityCache> drones)
        {
            try
            {
                ...
                ignoreNewTarget = ignoreNewTarget || secondsToKillCurrentTargetWithDrones <= 15 || hasCurrentTargetHighLocalReps || timeForDronesToGetToTheNewTarget > secondsToKillCurrentTargetWithDrones;
