    ...
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        internal bool? DoWeNeedToBuyItems
        {
            get
            {
                try
                {
                    ...
                    Log("if (ESCache.Instance.DirectEve.Me != null || ESCache.Instance.DirectEve.Me.Wealth == null)");
                    ...
                    if (BuildBuyList().Any())
                    {
                        ...
                        if (ESCache.Instance.DirectEve.Me.Wealth != null && (200_000_000 > ESCache.Instance.DirectEve.Me.Wealth) && ESCache.Instance.ItemHangar.Items.Where(i => i.IsAbyssalLootToSell).Sum(x => x.AveragePrice() * Math.Max(1, x.Quantity)) > 50_000_000)
                        {
                            Log("WalletBalance [" + ESCache.Instance.DirectEve.Me.Wealth + "] is lower than 200mil, we have loot to sell and BuildBuyList contains items: lets go sell stuff first");
        ...
        private Dictionary<int, int> BuildBuyList()
        {
            ...
            foreach (var item in _shipsCargoBayList)
            {
                ...
                var minMultiplier = 2;
                var maxMultiplier = Rnd.Next(3, 6);
                ...
                if (typeId == _naniteRepairPasteTypeId)
                {
                    ...
                    {
                        maxMultiplier = Rnd.Next(3, 5);
                    }
                    else if (ESCache.Instance.ActiveShip.IsDestroyer)
                    {
                        maxMultiplier = Rnd.Next(5, 8);
                    }
                    else
                    {
                        maxMultiplier = Rnd.Next(8, 13);
                    }
                }

                if (typeId == _filamentTypeId)
                    maxMultiplier = Rnd.Next(9, 12);
            ...
            if (_droneBayItemList != null &&_droneBayItemList.Any())
            {
                foreach (var item in _droneBayItemList)
                {
                    ...
                    var amount = item.Item2;
                    var minMultiplier = 1;
                    var maxMultiplier = Rnd.Next(2, 4);

                    // Skip mutated drones, as they are not avail on the market
                    if (typeId == 60480 || typeId == 60479 || typeId == 60478)
                        continue;

                    var countInHangarAndDroneBay = GetAmountofTypeIdLeftItemhangarAndDroneBay(typeId);
            ...
            if (_boosterList != null && _boosterList.Any())
            {
                foreach (var item in _boosterList)
                {
                    ...
                    var minMultiplier = 1;
                    var maxMultiplier = Rnd.Next(2, 5);
        ...
        private void TravelToMarketSystem()
        {
            ...
            try
            {
                ...
                Traveler.TravelToStationId(StationIDJitaP4M4);
            }
            catch (Exception ex)
            {
                Log("Exception [" + ex + "]");
