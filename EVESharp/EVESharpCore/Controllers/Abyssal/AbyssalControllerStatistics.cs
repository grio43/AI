...
namespace EVESharpCore.Controllers.Abyssal
{
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        public void WriteStatsToDB()
        {
            ...
            if (_abyssStatEntry != null)
            {
                Log("WriteStatsToDB: Disabled: FixMe: There was some issue with the dependencies / References that need attention in this branch");
                /*
        ...
        internal void ManageStats()
        {
            ...
            var largeDronesLeftInbay = _droneBayItemList.Where(x => x.Item3 == 3).Sum(x => GetAmountOfTypeIdLeftInDroneBay(x.Item1));
            var medDronesLeftInbay = _droneBayItemList.Where(x => x.Item3 == 2).Sum(x => GetAmountOfTypeIdLeftInDroneBay(x.Item1));
            var smallDronesLeftInbay = _droneBayItemList.Where(x => x.Item3 == 1).Sum(x => GetAmountOfTypeIdLeftInDroneBay(x.Item1));
            ...
            switch (CurrentAbyssalStage)
            {
                case AbyssalStage.Stage1:
                    ...
                    else
                    {
                        if (noDronesInSpaceAndNoLimboDrones)
                            _abyssStatEntry.LostDronesRoom1 = amountOfAllDronesInBayAfterArm - amountOfAllDronesInBay - allDronesInSpace.Count;
                ...
                case AbyssalStage.Stage3:
                    ...
                    {
                        if (noDronesInSpaceAndNoLimboDrones)
                            _abyssStatEntry.LostDronesRoom3 = amountOfAllDronesInBayAfterArm - amountOfAllDronesInBay - _abyssStatEntry.LostDronesRoom1 - _abyssStatEntry.LostDronesRoom2 - allDronesInSpace.Count;
                        _abyssStatEntry.LargeDronesLost = _droneBayItemList.Where(x => x.Item3 == 3).Sum(x => x.Item2) - largeDronesLeftInbay - largeDronesInSpace.Count;
                        _abyssStatEntry.MediumDronesLost = _droneBayItemList.Where(x => x.Item3 == 2).Sum(x => x.Item2) - medDronesLeftInbay - mediumDronesInSpace.Count;
                        _abyssStatEntry.SmallDronesLost = _droneBayItemList.Where(x => x.Item3 == 1).Sum(x => x.Item2) - smallDronesLeftInbay - smallDronesInSpace.Count;
