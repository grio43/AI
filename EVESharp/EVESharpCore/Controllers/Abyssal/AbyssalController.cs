...
namespace EVESharpCore.Controllers.Abyssal
{
    ...
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        private double _drugsBoostersMinShieldPerc = .35;
        ...
        **/
        ...
        private List<(int, int)> _shipsCargoBayList
        {
            get
            {
                ...
                if (Defense.GlobalAllowOverLoadOfHardeners || Defense.GlobalAllowOverLoadOfReps || Defense.GlobalAllowOverLoadOfSpeedMod)
                {
                    if (ESCache.Instance.ActiveShip.IsFrigate)
                    {
                        if (_naniteRepairPasteTypeId != 0) __shipsCargoBayList.Add((_naniteRepairPasteTypeId, 75));
                    }
                    else if (ESCache.Instance.ActiveShip.IsDestroyer)
                    {
                        if (_naniteRepairPasteTypeId != 0) __shipsCargoBayList.Add((_naniteRepairPasteTypeId, 175));
                    }
                    else
                    {
                        //Add nanite repair paste if we are overheating
                        if (_naniteRepairPasteTypeId != 0) __shipsCargoBayList.Add((_naniteRepairPasteTypeId, 275));
                    }
                }
                ...
                if (ESCache.Instance.ActiveShip.TypeId == (int)TypeID.Hawk)
                {
                    ...
                    {
                        if (_capBoosterTypeId != 0) __shipsCargoBayList.Add((_capBoosterTypeId, Settings.Instance.NumberOfCapBoostersToLoad));
                    }
                }
                ...
                else __shipsCargoBayList.Add(((int)_filamentTypeId, numOfFilamentsToBring));
                ...
                foreach (var individualDefinedAmmoType in DirectUIModule.DefinedAmmoTypes.Distinct())
                {
                    ...
                    if (individualDefinedAmmoType.TypeId != 0) __shipsCargoBayList.Add((individualDefinedAmmoType.TypeId, individualDefinedAmmoType.Quantity));
                }
        ...
        /// </summary>
        ///
        private static List<(int, int, int)> __droneBayItemList = null;
        private static List<(int, int, int)> _droneBayItemList
        {
            get
            {
                if (__droneBayItemList == null)
                {
                    if (ESCache.Instance.DirectEve.ActiveShip.TypeId == (int)TypeID.Gila)
                    {
                        ...
                        //__droneBayItemList.Add(_mediumDroneTypeId, 10, 2);
                        __droneBayItemList = new List<(int, int, int)>
                        {
                            ((int)_mediumDroneTypeId, 10, 2),
                            //((int)_smallDroneTypeId, 4, 1)
                        };
                    }
                    else if (ESCache.Instance.DirectEve.ActiveShip.TypeId == (int)TypeID.Ishtar)
                    {
                        //__droneBayItemList = new List<(int, int, int)>();
                        //__droneBayItemList.Add(_geckoDroneTypeId, 3, 3);
                        //__droneBayItemList.Add(_heavyDroneTypeId, 6, 3);
                        //__droneBayItemList.Add(_mediumDroneTypeId, 5, 2);
                        //__droneBayItemList.Add(_smallDroneTypeId, 5, 1);
                        __droneBayItemList = new List<(int, int, int)>
                        {
                            ((int)_geckoDroneTypeId, 6, 3),
                            //((int)_heavyDroneTypeId, 6, 3),
                            ((int)_mediumDroneTypeId, 5, 2),
                            ((int)_smallDroneTypeId, 5, 1),
                        };
                    }
                    else if (ESCache.Instance.DirectEve.ActiveShip.TypeId == (int)TypeID.VexorNavyIssue)
                    {
                        ...
                        __droneBayItemList = new List<(int, int, int)>
                        {
                            ((int)_heavyDroneTypeId, 5, 3), //125m3
                            ((int)_mediumDroneTypeId, 7, 2),//70m3
                            ((int)_smallDroneTypeId, 0, 1), //0m3
                        };
                    }
        ...
        /**
        private static List<(int, int, int)> _droneBayItemList = new List<(int, int, int)>
        {
            (_geckoDroneTypeId, 3, 3),
            (_heavyDroneTypeId, 6, 3),
            (_mediumDroneTypeId, 5, 2),
            (_smallDroneTypeId, 5, 1),
        };
        **/

        // [large = 3, med = 2, small = 1] 3x gecko (33681) , 6x Mutated Berserkers (2478), 5 Republic Fleet Valkyrie (21640), 5 Republic Fleet Warrior (2488)

        //private static List<(int, int, int)> _droneBayItemList = new List<(int, int, int)> { (_geckoDroneTypeId, 3, 3), (_karenDroneTypeId, 6, 3), (_mediumDroneTypeId, 5, 2), (_smallDroneTypeId, 5, 1) };  // [large = 3, med = 2, small = 1] 3x gecko (33681) , 6x Mutated Berserkers (2478), 5 Republic Fleet Valkyrie (21640), 5 Republic Fleet Warrior (2488)

        // heavy mutated = 60480
        // private List<(int, int, int)> _droneBayItemList = new List<(int, int, int)> { (33681, 5, 3), (31890, 7, 2), (31888, 11, 1) };  // [large = 3, med = 2, small = 1] 3x gecko (33681) , 6x T2 Berserkers (2478), 5 Republic Fleet Valkyrie (21640), 5 Republic Fleet Warrior (2488)
        ...
        public static bool AllowOverloadOfHardener = true;
        ...
        internal bool overHeatReps
        {
            get
            {
                ...
                {
                    if (DirectEve.Modules.Any(e => e.HeatDamagePercent >= heatDamageMaximum_RepairModule))
                    {
        ...
        internal bool overHeatHardener
        {
            get
            {
                ...
                if (DirectEve.HasFrameChanged())
                {
                    var Hardeners = DirectEve.Modules.Where(e => e.GroupId == (int)Group.ShieldHardeners || e.GroupId == (int)Group.ArmorHardeners);

                    foreach (var item in Hardeners)
                    {
                        if (item.HeatDamagePercent > heatDamageMaximum_Hardener - 5)
                        {
        ...
        internal bool ManageOverheat()
        {
            ...
            var propMods = DirectEve.Modules.Where(e => e.GroupId == (int)Group.Afterburner);
            var capacitorInjector = DirectEve.Modules.Where(e => e.GroupId == (int)Group.CapacitorInjector);
            var Hardeners = DirectEve.Modules.Where(e => e.GroupId == (int)Group.ShieldHardeners || e.GroupId == (int)Group.ArmorHardeners);
            var repairers = DirectEve.Modules.Where(e => e.GroupId == (int)Group.ShieldBoosters || e.GroupId == (int)Group.ArmorRepairer);
            ...
            if (overHeatHardener && Hardeners.Any(e => e.IsBeingRepaired))
            {
                if (DebugConfig.DebugOverLoadHardeners) Log("if (overHeatHardener && shieldHardener.Any(e => e.IsBeingRepaired))");
                foreach (var item in Hardeners.Where(e => e.IsBeingRepaired))
                {
                    if (item.CancelRepair())
                    {
            ...
            if (overHeatPropmod && propMods.Any(e => e.IsBeingRepaired))
            {
                if (DebugConfig.DebugOverLoadHardeners) Log("if (overHeatPropmod && afterBurner.Any(e => e.IsBeingRepaired))");
                foreach (var item in propMods.Where(e => e.IsBeingRepaired))
                {
                    if (item.CancelRepair())
                    {
                ...
                if (shipsCargo.Items.Any(i => i.TypeId == _naniteRepairPasteTypeId))
                {
                    if (passiveModuleHighestDamagePercent >= passiveModuleStartRepairPercent && passiveModules.Any())
                    {
                        ...
                        foreach (var passiveModule in passiveModules)
                        {
                            ...
                            if (!passiveModule.IsBeingRepaired && !passiveModule.IsInLimboState && passiveModule.HeatDamagePercent >= passiveModuleStartRepairPercent)
                            {
                                if (!ShouldWeRepair)
                                    continue;

                                if (passiveModule.Repair())
                                {
            ...
            foreach (var hardener in Hardeners)
            {
                ...
                //fixme
                if ((hardener.HeatDamagePercent > heatDamageMaximum_Hardener || passiveModuleThresholdExceeded) &&
                    hardener.IsOverloaded && hardener.ToggleOverload())
                {
                    Log(
                        $"Disabling overload on [{hardener.TypeName}] Heatdamage is too high! heatDamageMaximum_Hardener [{heatDamageMaximum_Hardener}] Current [{hardener.HeatDamagePercent}]");
                    return true;
                }

                if (modules.Any(e => e.ItemId != hardener.ItemId && e.HeatDamagePercent > heatDamageMaximum_RepairModule) &&
                    hardener.IsOverloaded && hardener.ToggleOverload())
                {
                    var damagedModule = modules.FirstOrDefault(e => e.HeatDamagePercent > heatDamageMaximum_RepairModule);
                    Log($"Disabling overload on [" + hardener.TypeName + "] Heatdamage is too high on other module [" + damagedModule.TypeName + "]! heatDamageMaximum_RepairModule [" + heatDamageMaximum_RepairModule + "] Current [" + damagedModule.HeatDamagePercent + "]");
                    return true;
                }
            }
            ...
            foreach (var repairer in repairers)
            {
                ...
                //fixme
                if ((repairer.HeatDamagePercent > heatDamageMaximum_RepairModule || passiveModuleThresholdExceeded) &&
                    repairer.IsOverloaded && repairer.ToggleOverload())
                {
                    Log(
                        $"Disabling overload on [{repairer.TypeName}] Heatdamage is too high! Max [{heatDamageMaximum_RepairModule}] Current [{repairer.HeatDamagePercent}]");
                    return true;
                }

                if (modules.Any(e => e.ItemId != repairer.ItemId && e.HeatDamagePercent > heatDamageMaximum_RepairModule) &&
                    repairer.IsOverloaded && repairer.ToggleOverload())
                {
                    var damagedModule = modules.FirstOrDefault(e => e.HeatDamagePercent > heatDamageMaximum_RepairModule);
                    Log($"Disabling overload on [" + repairer.TypeName + "] Heatdamage is too high on other module [" + damagedModule.TypeName + "]! Max [" + heatDamageMaximum_RepairModule + "] Current [" + damagedModule.HeatDamagePercent + "]");
                    return true;
                }
            }
            ...
            foreach (var propmod in propMods)
            {
                ...
                //fixme
                if ((propmod.HeatDamagePercent > heatDamageMaximum_Propmod || passiveModuleThresholdExceeded) &&
                    propmod.IsOverloaded && propmod.ToggleOverload())
                {
                    Log(
                        $"Disabling overload on [{propmod.TypeName}] Heatdamage is too high! Max [{heatDamageMaximum_RepairModule}] Current [{propmod.HeatDamagePercent}]");
                    return true;
                }

                if (modules.Any(e => e.ItemId != propmod.ItemId && e.HeatDamagePercent > heatDamageMaximum_RepairModule) &&
                    propmod.IsOverloaded && propmod.ToggleOverload())
                {
                    var damagedModule = modules.FirstOrDefault(e => e.HeatDamagePercent > heatDamageMaximum_RepairModule);
                    Log($"Disabling overload on [" + propmod.TypeName + "] Heatdamage is too high on other module [" + damagedModule.TypeName + "]! Max [" + heatDamageMaximum_RepairModule + "] Current [" + damagedModule.HeatDamagePercent + "]");
                    return true;
                }
            }
            ...
            if (overHeatHardener)
            {
                ...
                foreach (var item in Hardeners)
                {
                    if (!item.IsOverloaded)
                    {
                        ...
                        if (item.HeatDamagePercent < heatDamageMaximum_RepairModule - 5)
                        {
                            if (DebugConfig.DebugOverLoadHardeners) Log("if (item.HeatDamagePercent < heatDamageMaximum - 5)");
                            if (item.ToggleOverload())
                            {
                                Log($"Overheating hardener [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] IsActive [" + item.IsActive + "]");
                                continue;
                            }
                            else Log($"Overheating hardener [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] Overload failed?!");

                            continue;
                        }

                        if (DirectEve.Interval(5000)) Log($"Overheating hardener [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] > heatDamageMaximum_RepairModule [" + heatDamageMaximum_RepairModule + "] - 5% Not overheating");
                        continue;
                    }
                ...
                foreach (var item in capacitorInjector)
                {
                    if (!item.IsOverloaded)
                    {
                        ...
                        if (item.HeatDamagePercent < heatDamageMaximum_RepairModule - 5)
                        {
                            ...
                            if (item.ToggleOverload())
                            {
                                Log($"Overheating capacitorInjector [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] IsActive [" + item.IsActive + "]");
                                continue;
                            }
                            else Log($"Overheating capacitorInjector [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] Overload failed?!");

                            continue;
                        }

                        if (DirectEve.Interval(5000)) Log($"Overheating capacitorInjector [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] Damage too high! Not overheating");
                        continue;
                    }
                ...
                foreach (var item in repairers)
                {
                    if (!item.IsOverloaded)
                    {
                        if (item.HeatDamagePercent < heatDamageMaximum_RepairModule - 5)
                        {
                            if (item.ToggleOverload())
                            {
                                Log($"Overheating booster [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] IsActive [" + item.IsActive + "]");
                                continue;
                            }
                        ...
                        Log($"Overheating booster [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] Damage too high! Not overheating");
                        continue;
                    }
            ...
            if (overHeatPropmod)
                foreach (var item in propMods)
                {
                    if (!item.IsOverloaded)
                    {
                        if (item.HeatDamagePercent < heatDamageMaximum_Propmod - 7)
                        {
                            if (item.ToggleOverload())
                            {
                                Log($"Overheating speedmod [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] overHeatPropmod [" + overHeatPropmod + "] IsActive [" + item.IsActive + "]");
                                continue;
                            }
                        ...
                        Log($"Overheating speedmod [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] overHeatPropmod [" + overHeatPropmod + "] Damage too high! Not overheating");
                        continue;
                    }
            ...
            if (!overHeatPropmod)
                foreach (var item in propMods)
                {
                    ...
                    {
                        Log($"Disabling overload on [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] overHeatPropmod [" + overHeatPropmod + "]");
                        return true;
                    }
                }
            ...
            if (!overHeatHardener)
            {
                ...
                foreach (var item in Hardeners)
                {
                    if (item.IsOverloaded && item.ToggleOverload())
                    {
                        Log($"Disabling overload on [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] overHeatHardener [" + overHeatHardener + "]");
                        return true;
                    }
                }
            }
            ...
            if (!overHeatReps)
                foreach (var item in repairers)
                {
                    ...
                    {
                        Log($"Disabling overload on [{item.TypeName}] Heatdamage [{Math.Round(item.HeatDamagePercent, 0)}] overHeatBooster [" + overHeatReps + "]");
                        return true;
                    }
                }
        ...
        internal bool boolWeNeedBoostersNow
        {
            get
            {
                if (ESCache.Instance.InSpace)
                {
                    if (ESCache.Instance.InAbyssalDeadspace)
                    {
                        if (ESCache.Instance.EveAccount.UseFleetMgr && _drugsBoostersMinShieldPerc > ESCache.Instance.ActiveShip.Entity.ShieldPct)
                            return true;
        ...
        internal bool ManageHardeners()
        {
            ...
            foreach (var hardener in DirectEve.Modules.Where(e => e.GroupId == (int)Group.ShieldHardeners || e.GroupId == (int)Group.ArmorHardeners))
            {
                ...
                if (hardener.IsActive)
                {
                    if (boolShouldWeDeactivateHardener)
                    {
                        ...
                        {
                            if (DebugConfig.DebugDefense) Log("[" + hardener.TypeName + "] IsInLimboState [" + hardener.IsInLimboState + "] IsOnline [" + hardener.IsOnline + "] IsActivatable [" + hardener.IsActivatable + "] IsDeactivating [" + hardener.IsDeactivating + "] IsReloadingAmmo [" + hardener.IsReloadingAmmo + "] IsBeingRepaired [" + hardener.IsBeingRepaired + "] EffectActivating [" + hardener.EffectActivating + "] IsEffectActivating(this) [" + DirectEve.IsEffectActivating(hardener) + "] ReactivationDelay [" + hardener.ReactivationDelay + " > 0]");
                            continue;
                        }
                    }
                }
                ...
                if (!AllRatsHaveBeenCleared)
                {
                    if (hardener.IsBeingRepaired)
                    {
                        // cancel repair
                        if (hardener.CancelRepair())
                        {
                            if (!AllRatsHaveBeenCleared)
                            {
                ...
                {
                    if (!hardener.IsBeingRepaired && hardener.HeatDamagePercent > 5)
                    {
                        var shipsCargo = ESCache.Instance.DirectEve.GetShipsCargo();
        ...
        internal bool ManageShieldRepairers()
        {
            ...
            if (shieldRepairModules.Any())
            {
                foreach (var repairer in shieldRepairModules.OrderBy(x => x.TypeName.Contains("Gistum B-Type")).ThenBy(x => x.TypeName.Contains("Pithum C-Type")).ThenBy(x => x.ItemId))
                {
                    ...
                    {
                        if (!repairer.IsBeingRepaired && repairer.HeatDamagePercent > 5)
                        {
                            var shipsCargo = ESCache.Instance.DirectEve.GetShipsCargo();
        ...
        public static void OverloadModules(bool allowOverLoadOfTheseModules, List<ModuleCache> modulesToTryToOverload, int moduleDamagePercentageAllowed, int targetDistanceToUnOverload = 99999, EntityCache myTarget = null)
        {
            try
            {
                ...
                foreach (ModuleCache overloadedModule in modulesToTryToOverload.Where(i => i.IsOverloaded && !i.IsPendingStopOverloading && !i.IsPendingOverloading))
                {
                    ...
                    if (ESCache.Instance.ActiveShip.TypeId == (int)TypeID.StormBringer || ESCache.Instance.ActiveShip.TypeId == (int)TypeID.Retribution)
                    {
                        if (HighRackHeatDisableThreshold > highRackHeatStatus && moduleDamagePercentageAllowed > overloadedModule.DamagePercent)
                            continue;
                    }
                ...
                foreach (ModuleCache moduleToOverload in modulesToTryToOverload.Where(i => !i.IsOverloaded && !i.IsPendingStopOverloading && !i.IsPendingOverloading && !i._module.IsBeingRepaired))
                {
                    ...
                    if (highRackHeatStatus > HighRackHeatDisableThreshold)
                    {
                        if (DebugConfig.DebugOverLoadWeapons) Log("if (highRackHeatStatus [" + highRackHeatStatus + "] > HighRackHeatDisableThreshold [" + HighRackHeatDisableThreshold + "])");

                        continue;
                    }

                    //If overload is not active, only activate it if the rack heat is below the highRackHeatEnableThreshold
                    if (!moduleToOverload.IsOverloaded && highRackHeatStatus > HighRackHeatDisableThreshold)
                    {
                        if (DebugConfig.DebugOverLoadWeapons) Log("if (!moduleToOverload.IsOverloaded && highRackHeatStatus [" + highRackHeatStatus + "] > HighRackHeatDisableThreshold [" + HighRackHeatDisableThreshold + "])");
                    ...
                    {
                        if (DebugConfig.DebugOverLoadWeapons) Log("if (myTarget != null && targetDistanceToUnOverload != 99999 && targetDistanceToUnOverload [" + targetDistanceToUnOverload + "] > myTarget.Distance [" + myTarget.Distance + "])");
                    ...
                    {
                        if (DebugConfig.DebugOverLoadWeapons) Log("if (moduleToOverload.DamagePercent [" + moduleToOverload.DamagePercent + "] >= moduleDamagePercentageAllowed [" + moduleDamagePercentageAllowed + "])");
        ...
        public static bool boolOverLoadWeapons
        {
            get
            {
                //if already overloaded turn overload off for a cycle to save heat!
                ...
                if (AbyssalSpawn.DetectSpawn == AbyssalSpawn.AbyssalSpawnType.ConcordSpawn)
                {
                    if (Combat.PotentialCombatTargets.Any(i => i.IsTarget && i.IsNPCCruiser && i.IsReadyToShoot))
                        return true;

                    if (Combat.PotentialCombatTargets.Any(i => i.IsTarget && i.IsNPCBattleship && i.IsReadyToShoot))
                        return true;
                }
        ...
        internal bool ManageWeapons()
        {
            try
            {
                ...
                //Cancel Repair if needed
                if (ESCache.Instance.Targets.Any(i => i.Velocity > 0))
                {
                ...
                {
                    //Repair if needed
                    if (DirectEve.Me.IsInAbyssalSpace() && IsOurShipWithintheAbyssBounds() && (Combat.PotentialCombatTargets.All(i => !i.IsAbyssalBioAdaptiveCache) || allDronesInSpace.Any()))
                    {
                        if (ESCache.Instance.DirectEve.GetShipsCargo() != null)
                        {
                            ...
                            {
                                foreach (var thisWeapon in ESCache.Instance.Weapons.Where(i => !i._module.IsBeingRepaired && i._module.HeatDamagePercent > 0))
                                {
                                    if (!ShouldWeRepair)
                                        continue;
                ...
                if (ESCache.Instance.ActiveShip.HasDroneBay)
                {
                    ...
                    if (currentDroneTargets != null && currentDroneTargets.Any())
                    {
                        ...
                        if (DirectEve.Interval(400, 400, weapon.ItemId.ToString()))
                        {
                            ...
                            if (weapon.Activate(currentDroneTargets.FirstOrDefault().Id))
                            {
                                return true;
                            }
                        }
