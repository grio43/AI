        ...
        public void ProcessState()
        {
            try
            {
                ...
                if (AgentInteraction.boolSwitchAgents)
                {
                    ...
                    ESCache.Instance.CloseEveReason = "Done with this caeeragent: restart eve to kick the bot into using next agent (bad hack)";
        ...
        public bool CareerAgentPrepareToMoveAgents()
        {
            ...
            if (ESCache.Instance.DirectEve.Session.StationId != JitaP4M4Station)
            {
                ...
                List<DirectItem> ships = ESCache.Instance.ShipHangar.Items.Where(x => x.GroupId == (int)Group.Frigate &&
                                                                                      x.Quantity == 1 &&
                                                                                      x.GivenName == null &&
                                                                                      (x.TypeName.ToLower() == "Slasher".ToLower() ||
                                                                                       x.TypeName.ToLower() == "Rifter".ToLower()  ||
                                                                                       x.TypeName.ToLower() == "Slasher".ToLower() ||
                                                                                       x.TypeName.ToLower() == "Slasher".ToLower() ||
                                                                                       x.TypeName.ToLower() == "Slasher".ToLower() ||
                                                                                       x.TypeName.ToLower() == "Slasher".ToLower())
